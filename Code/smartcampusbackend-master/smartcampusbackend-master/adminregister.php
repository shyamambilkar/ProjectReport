<?php
	...
	if(isset($_POST['fullname']) && isset($_POST['username']) && isset($_POST['email']) && isset($_POST['password']) && isset($_POST['confirmpassword'])) {
		...
		if($db->checkifuserexistedAdmin($username)){
			...
			if($isValid){
				if ($password == $confirmpassword) {
					$user = $db->storeUsersDataAdmin($fullname, $username, $email, $password, $confirmpassword); //create new user
				   if($user != false){
					   	$verify = $db->sendemailverifyAdmin($email, $username, $password); //send email for verification
