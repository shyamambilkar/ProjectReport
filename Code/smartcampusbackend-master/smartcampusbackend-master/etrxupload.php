<?php
	...
	if (isset($_POST['name']) && isset($_FILES['file']['name'])) {
		...
		$file_upload_url = 'http://' . $server_ip.'/'.'smartcampus'.'/'.$target_path;
		...
		try {
				...
				if(in_array($fileext,$ext_arr)) {
					// Throws exception incase file is not being moved
						...
						if($db->checkFileEtrxExists($name, $file_upload_url)){
							$reponse['error'] = FALSE;
							$response['error_msg'] = 'File already exists';
						}else{
							$storedata = $db->storeEtrxUploadData($name, $file_upload_url);
							if($storedata){
